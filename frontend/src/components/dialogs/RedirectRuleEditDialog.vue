<template>
  <q-dialog @show="show()" @hide="hide()" full-width v-model="is_open" :position="position">
    <q-card v-if="redirect_rule != null" class="bg-grey-3 modal">
      <q-card-section class="bg-primary text-white">
        <div class="row flex-center">
          <div class="col">
            <div class="text-h6">EDIT | Redirect Rule</div>
          </div>
          <div class="col-auto">
            <q-icon name="edit" size="35px"/>
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-section>
        <div class="doc-note doc-note--tip">
          ...
          <p>
            Once a rule is updated it will <b>NOT</b> take effect instantly.
        ...
        <q-card class="text-h6">
          <div class="row q-item">
            <div class="col">
              EDITING REDIRECT RULE WITH ID:
            </div>
            <div class="col-auto text-bold text-primary">
              {{ redirect_rule.id }}
            </div>
          </div>
        </q-card>
      ...
      <q-card-actions>
        <q-btn color="green" @click="update()">UPDATE</q-btn>
        <q-btn color="secondary" @click="show()">RESET TO ORIGINAL</q-btn>
        <q-btn @click="hide()" color="negative" label="CLOSE / CANCEL" flat></q-btn>
      </q-card-actions>
    </q-card>
  </q-dialog>
</template>
...
<script>
export default {
  name: 'RedirectRuleEditDialog',
  computed: {
    is_open: {
      get: function () {
        return this.$store.state.dialogs.show_edit
      },
      set: function (value) {
        this.$store.commit('dialogs/set_edit', value)
      }
    },
    redirect_rule () {
  ...
  methods: {
    update () {
      // Gather data
      this.weight = parseInt(this.weight.toString(), 10)

      let postData = {
        redirect_rule_id: this.redirect_rule.id,
        domain: this.domain,
      ...
      // Make API call
      this.$axios.post(this.$store.state.api.MGMT_RULES_UPDATE, postData)
    ...
    handle_response (response) {
      ...
      this.$q.notify({
        color: 'green',
        position: 'top',
        message: 'Redirect Rule with ID: ' + data.updated_rule.id + ' successfully updated!',
        icon: 'check',
    ...
    show () {
      this.domain = this.redirect_rule.domain_rule.rule
      this.domain_is_regex = this.redirect_rule.domain_rule.is_regex ? 'REGEX' : 'LITERAL'

      this.path = this.redirect_rule.path_rule.rule
      this.path_is_regex = this.redirect_rule.path_rule.is_regex ? 'REGEX' : 'LITERAL'

      this.destination = this.redirect_rule.destination_rule.destination_url
      this.destination_is_rewrite = this.redirect_rule.destination_rule.is_rewrite ? 'REWRITE' : 'LITERAL'

      this.weight = this.redirect_rule.weight
    },
    hide () {
      this.clear()
      this.is_open = false
